...
# Kleurtoewijzing per transportcategorie
    ...
    for node1, node2 in G.edges():
        category = G[node1][node2].get('category', 'onbekend')
    ...
    solid = mlines.Line2D([], [], color='black', linestyle='solid', label='Bestaande verbinding')
    dashed = mlines.Line2D([], [], color='black', linestyle='dashed', label='Nieuwe verbinding')
...
def edge_cost(G, beta, upc=0, cpc=1, transport_parameters=None, allowed_edge_transitions=None):
    ...
    # ==== Nieuw: Componenten van zuiver nieuwe Road-edges bepalen ====
    # ===== Nieuw: bepaal componenten van nieuwe road-edges =====
    new_road_edges = [
        (u, v) for u, v, d in G1.edges(data=True)
        if d.get('category') == 'Road' and 'current' not in d
    ]
    subG_new_roads = G1.edge_subgraph(new_road_edges).copy()
    road_components = list(nx.connected_components(subG_new_roads))

    def is_first_edge_in_component(node1, node2):
        """Check of deze edge de 'eerste' is in zijn nieuwe-road-component."""
        for comp in road_components:
            if node1 in comp and node2 in comp:
                # Sorteer nodes zodat volgorde vast ligt
                min_node = min(comp)
                return node1 == min_node or node2 == min_node
            return True  # Als geen component → behandel als eerste

    # ==== Normale edge_cost-logica ====
    for node1, node2 in G1.edges():
        weight = G1[node1][node2]['weight']
        capacity = G1[node1][node2]['capacity']
        category = G1[node1][node2].get('category', 'onbekend')

        if 'current' not in G1[node1][node2]:  # nieuwe verbinding
            if beta == 0 and capacity == 0:
                G1[node1][node2]['cost'] = 0
                G1[node1][node2]['category'] = 'onbekend'
                capex = 0
                opex = 0
            else:
                best_cost = float('inf')
                ...
                for edge_type in allowed_edge_transitions:
                    if edge_type == 'Pipeline':
                        ...
                        N_truck_purchase = math.ceil(N_truckloads / 365)
                        N_times_new_trucks = rekenperiode / lifetime_truck

                        # Nieuw: alleen eerste edge in component krijgt capex
                        if is_first_edge_in_component(node1, node2):
                            capex = (N_truck_purchase * N_times_new_trucks * invest_truck)/rekenperiode
                        else:
                            capex = 0

                        opex_fix = capex * opex_fix_truck
                        opex_var = cost_var_truck*weight*2*N_truckloads
                        opex = opex_fix + opex_var
                        cost = capex + opex
                        temp_N_vehicle = N_truck_purchase if capex > 0 else 0
            ...
            if category == 'Pipeline':
                ...
                gewicht_H2 = capacity / (E_GH2 * behoud_energie) / 365
                capex = 0
                opex = (50833 * weight * math.e**(0.0697 * diameter) + 297 * (diameter**2) + 71800 * diameter + 54658) * opex_fix_pipeline
                ...
                gewicht_H2 = capacity / E_LH2
                N_truckloads = math.ceil(gewicht_H2 / cap_truck)
                capex = 0  # bestaande weg → geen aanschafkosten
                opex_fix = capex * opex_fix_truck
                opex_var = cost_var_truck_snelweg * weight * 2 * N_truckloads
                opex = opex_fix + opex_var
                G1[node1][node2]['cost'] = round(opex, 2)
                G1[node1][node2]['N_vehicle'] = N_truckloads
        ...
        # --- Debug: Road componenten bekijken ---
    new_road_edges = [
        (u, v) for u, v, d in G1.edges(data=True)
        if d.get('category') == 'Road' and 'current' not in d
    ]
    ...
    print("Aantal Road-componenten:", len(road_components))
    for comp in road_components:
        edges_in_comp = [
            (u, v) for u, v in subG_new_roads.edges()
            if u in comp and v in comp
        ]
        print("Component nodes:", comp)
        print("Component edges:", edges_in_comp)
    # --- Einde debug ---
    return G1
